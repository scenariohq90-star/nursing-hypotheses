...
- Responsive evidence: `artifacts/qa/question-bank-mobile-ar-390x720-normalized.png`
...
- Mobile implementation: 390 x 720 rendered pixels, 390px CSS content width, device pixel ratio 1. The in-app browser was set to 405 x 748 to normalize its content capture. `scrollWidth` equalled `clientWidth` at 390px.
...
- **Interaction states:** scenario choice, confirm, feedback and next-step states work; context details expand; question-bank validation, locked answers, final debrief and local analytics work; mobile navigation, language switching, references and membership-preview routes work.
- **Responsiveness and accessibility:** Arabic RTL and English LTR were checked. At 390px there was no horizontal overflow. Controls are semantic buttons, radios, links, details/summary and progress elements; focus styles and screen-reader labels remain present.
- **Console:** final in-app browser inspection returned no warnings or errors.
...
- [x] 24 content/engine tests, production build and 4 Sites-worker tests passed.
