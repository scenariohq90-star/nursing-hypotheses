...
const copy = {
  en: {
    skip: "Skip to main content", home: "Home", simulations: "Scenarios", scenarios: "Scenarios", questionBank: "Question bank", dosePractice: "Medication math", learning: "My learning", resources: "References", about: "About",
    menu: "Open navigation", closeMenu: "Close navigation", language: "Language", learner: "Guest beta", localProfile: "Learning profile", localProfileShort: "This device", guestBetaEyebrow: "Public testing beta", guestBetaTitle: "No account needed", guestBetaBody: "Your progress stays in this browser. Account sync and the AI assistant are temporarily unavailable while their production safeguards are completed.", sendFeedback: "Send feedback",
    ...
    learningEyebrow: "Your learning profile", learningTitle: "See your practice pattern", learningBody: "Your dashboard summarises completed fictional scenarios and highlights where more practice may help. During this public beta, progress stays on this device and never certifies competence.",
    ...
    boundaries: "Clinical boundaries", boundariesBody: "It does not replace supervision, local policy, formal education, professional assessment or real-time clinical judgement.", accountModel: "Public beta access",
  ...
  ar: {
    skip: "انتقل إلى المحتوى الرئيسي", home: "الرئيسية", simulations: "السيناريوهات", scenarios: "السيناريوهات", questionBank: "بنك الأسئلة", dosePractice: "حساب الجرعات", learning: "تعلّمي", resources: "المراجع", about: "عن المنصة",
    menu: "فتح قائمة التنقل", closeMenu: "إغلاق قائمة التنقل", language: "اللغة", learner: "تجربة الضيف", localProfile: "ملف التعلم", localProfileShort: "هذا الجهاز", guestBetaEyebrow: "نسخة تجريبية عامة", guestBetaTitle: "لا تحتاج إلى حساب", guestBetaBody: "يبقى تقدمك داخل هذا المتصفح. مزامنة الحساب والمساعد الذكي غير متاحين مؤقتاً إلى أن تكتمل ضوابطهما التشغيلية.", sendFeedback: "أرسل ملاحظتك",
    privacyStrip: "نسخة تجريبية عامة · للضيف فقط. يبقى التقدم على هذا الجهاز، وتسجيل الحسابات والذكاء الاصطناعي متوقفان مؤقتاً. تعليم أولي فقط—لا تدخل بيانات مرضى ولا تستخدمه لرعاية مريض.", simulation: "محاكاة تعليمية", eyebrow: "تدرّب على الحكم السريري بأمان",
    ...
    learningEyebrow: "ملف تعلمك", learningTitle: "شاهد نمط تدريبك", learningBody: "تلخص لوحة التعلم السيناريوهات الخيالية المكتملة وتوضح أين قد يفيد مزيد من التدريب. يبقى التقدم على هذا الجهاز في النسخة العامة التجريبية، ولا تعتمد اللوحة الكفاءة.",
    ...
    boundaries: "الحدود السريرية", boundariesBody: "لا تحل المنصة محل الإشراف أو السياسة المحلية أو التعليم الرسمي أو التقييم المهني أو الحكم السريري الفوري.", accountModel: "دخول النسخة العامة التجريبية",
...
const POLICY_PAGES = {
  en: {
    privacy: {
      ...
      effective: "Effective 5 September 2026 · public testing notice under legal review",
      ...
      sections: [
        ...
        { title: "Information used", body: "The browser stores the selected language, scenario/question/option identifiers, completed attempts and the active timed session on this device. The medication-math page can accept weight, an authorised amount and product-label concentration for arithmetic, while its practice mode uses fixed fictional values. These numeric fields remain only in page memory until reset or navigation and are not saved to learning history. Do not enter names, record numbers or clinical notes. The current beta has no account registration, cloud sync, free-text assistant or payment collection. Scores and learning-domain signals are recalculated from the current authored content. Do not enter patient data, payment data, licence numbers or employer information." },
    ...
    terms: {
      ...
      sections: [
        { title: "Educational scope", body: "The website offers fictional scenarios and independently authored practice questions for self-study and facilitated discussion. The medication calculator performs arithmetic from user-entered weight, an authorised mg/kg/dose amount and a product-label concentration; named options populate concentration fields only. It does not choose a dose, validate an order or establish safe administration. The separate practice mode grades arithmetic against fixed fictional problems. The website does not provide medical advice, professional supervision, a credential, continuing-education credit, licensure eligibility, a competency decision or a prediction of any examination result." },
  ...
  ar: {
    privacy: {
      ...
      effective: "يسري من 5 سبتمبر 2026 · إشعار تجربة عامة قيد المراجعة القانونية",
      ...
      sections: [
        ...
        { title: "المعلومات المستخدمة", body: "يحفظ المتصفح على هذا الجهاز اللغة ومعرّفات السيناريوهات والأسئلة والخيارات والمحاولات المكتملة والجلسة المؤقتة النشطة. يمكن لصفحة الحساب الدوائي استقبال الوزن والكمية المعتمدة وتركيز ملصق المنتج لإجراء العملية الحسابية، بينما يستخدم وضع التدريب قيماً خيالية ثابتة. تبقى هذه الحقول الرقمية في ذاكرة الصفحة حتى المسح أو الانتقال ولا تُحفظ في سجل التعلم. لا تدخل أسماء أو أرقام ملفات أو ملاحظات سريرية. لا تتضمن النسخة الحالية تسجيل حساب أو مزامنة سحابية أو مساعداً للنص الحر أو تحصيل مدفوعات. لا تدخل بيانات مرضى أو بيانات دفع أو رقم ترخيص أو جهة عمل." },
...
function Header({ lang, setLanguage, route, menuOpen, setMenuOpen, t, auth, syncStatus, accountsEnabled }) {
  const menuButtonRef = useRef(null);
  const active = activeRoutePath(route);
  const accountLabel = auth.user?.email ?? (accountsEnabled ? t("localProfile") : t("learner"));
  ...
  return <>
    ...
    <header className="site-header">
      <div className="header-inner">
        ...
        <div className="header-actions">
          ...
          <button ref={menuButtonRef} type="button" className="menu-button" onClick={() => setMenuOpen((open) => !open)} aria-expanded={menuOpen} aria-controls="mobile-navigation" aria-label={menuOpen ? t("closeMenu") : t("menu")}>{menuOpen ? <X size={25} /> : <List size={25} />}</button>
        </div>
      </div>
    ...
    <div className="safety-strip" role="note"><ShieldWarning size={18} weight="fill" aria-hidden="true" /><span>{t("privacyStrip")}</span><a href="mailto:Scenario.hq90@gmail.com?subject=Nursing%20Hypotheses%20public%20beta%20feedback">{t("sendFeedback")}</a></div>
  </>;
}
