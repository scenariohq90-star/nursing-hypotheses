...
import simulationHeroImage from "./assets/simulation-hero-v2.webp";
...
const AccountPanel = lazy(() => import("./components/AccountPanel.jsx")
  .then((module) => ({ default: module.AccountPanel })));
const NursingAssistant = lazy(() => import("./components/NursingAssistant.jsx")
  .then((module) => ({ default: module.NursingAssistant })));
...
const accountsAvailable = releaseFeatures.learningAccounts;
...
const copy = {
  en: {
    ...
    menu: "Open navigation", closeMenu: "Close navigation", openTools: "Open tools", closeTools: "Close tools", language: "Language", learner: accountsAvailable ? "Learning account" : "Guest beta", localProfile: "Learning profile", localProfileShort: "This device", guestBetaEyebrow: "Public testing beta", guestBetaTitle: "No account needed", guestBetaBody: accountsAvailable ? "Keep using the site as a guest, or sign in to privately sync completed learning progress across devices. The AI assistant remains temporarily unavailable." : "Your progress stays in this browser. Account sync and the AI assistant are temporarily unavailable while their production safeguards are completed.", sendFeedback: "Send feedback",
    privacyStrip: accountsAvailable ? "Public testing beta · sign-in is optional. Guest progress stays on this device; signed-in learners may privately sync completed progress. AI is temporarily off. Never enter patient data." : "Public testing beta · guest access only. Progress stays on this device; accounts and AI are temporarily off. Draft education only — never enter patient data or use for patient care.",
    ...
    assistantComingSoonTitle: "Smart nursing assistant", assistantComingSoonLabel: "Coming soon", assistantComingSoonBody: "Ask a general nursing question. The assistant will search current sources and explain the answer in English and Arabic, with links you can review.",
    ...
    bilingual: "Truly bilingual", bilingualBody: "Switch the complete experience without losing your place.", privateDemo: accountsAvailable ? "Guest or account" : "Guest beta", privateDemoBody: accountsAvailable ? "Try every scenario and question bank without an account, or sign in to sync completed progress across your devices." : "Try every scenario and question bank without an account. Progress remains only in this browser.",
    ...
    libraryEyebrow: "Scenario library", libraryTitle: "Choose a clinical situation to practise", libraryBody: "Each case follows a changing patient journey. Complete the decisions, then review the reasoning behind every response.",
    ...
    learningEyebrow: "Your learning profile", learningTitle: "See your practice pattern", learningBody: "Your dashboard summarises completed scenarios and highlights the areas that would benefit from more practice.",
    ...
    localStorageTitle: "How progress is stored", localStorageBody: accountsAvailable ? "This browser keeps your language and active timed-question session on this device. If you sign in, eligible completed scenario and question-set records can sync to your private learning account; guest progress stays on this device unless you explicitly choose to merge it. Never enter patient data or free-text clinical notes, and sign out and clear site data after using a shared device." : "This browser keeps your language, completed learning identifiers and active timed-question session on this device. Nothing is synced to an account in this public beta. No patient data or free-text clinical response belongs here; clear site data after using a shared device.",
    clearHistory: "Clear learning history", clearHistoryConfirm: "Clear all completed learning attempts from this device? Your selected language and active practice sessions will be kept.", historyCleared: "Learning history cleared. Your language preference and active practice sessions were kept.", historyClearPending: "Learning history is being cleared. Your active practice is preserved; try finishing again in a moment.",
    questionBankEyebrow: "Nursing licensure practice", questionBankTitle: "Computer-based practice with transparent reasoning", questionBankBody: "Choose a nursing study path, then use guided practice or build your own fixed set. Every item includes bilingual reasoning after you lock your answer.",
    ...
    quizProgress: "Question progress", quizScore: "Practice score", correctAnswers: "Correct answers", quizDebriefEyebrow: "Question-bank debrief", quizDebriefTitle: "Review what you understood and what to revisit", quizIncomplete: "Complete the current question set to see its debrief.", localExamProgress: "Question practice history", localExamProgressBody: "Review completed question sets, answered items and performance by learning domain.", setsCompleted: "Completed sets", questionsAnswered: "Questions answered", categoryInsights: "Learning-domain review", morePractice: "Lower accuracy in this sample", developingKnowledge: "Mixed performance in this sample", strongKnowledge: "Higher accuracy in this sample", noExamAttempts: "Your first completed question set will appear here.",
    originalPracticeNotice: "Independent original practice only. No recalled, secure or official examination items are used, and scores do not predict an examination result, licensure or competence.", allDifficultyLevels: "All levels", questionsAvailable: "questions available", practiceSet: "Practice set", newVariation: "New variation", viewQuestionBank: "Open question bank", uniqueItems: "unique questions", completedSetsEvidence: "completed sets", earlyIndicator: "Early learning indicator", categoryEvidenceBody: "A learning-domain signal appears only after at least three unique questions across two completed sets.", contextVariant: "Practice context", contextDetails: "Context details", contextVariantNote: "Context changes presentation only; the clinical cues, scoring and best response stay consistent.", guidedScenarioTitle: "Guided scenario order", guidedScenarioBody: "Completed decisions reorder the library so relevant lower-scoring or safety-flagged learning domains appear more often near the top. New scenarios remain in the mix.", recommendedNext: "Recommended next", recommendationExplore: "Broaden the baseline", recommendationEvidence: "Gather more evidence", recommendationDevelopment: "Practise mixed-performance areas", recommendationReview: "Revisit a lower-accuracy area", recommendationSafety: "Review a safety-critical choice", adaptiveLearningTitle: "Suggested next practice", adaptiveLearningBody: "Suggestions use completed activity on this device to focus the next practice session.", openRecommendedScenario: "Open suggested scenario", startGuidedQuestions: "Start guided questions", examNonAffiliation: "Nursing Hypotheses is an independently developed educational resource. It is not issued, sponsored, endorsed, approved or administered by any nursing regulator, examination owner or test-delivery provider. It contains no recalled or secure examination items. Practice scores describe performance only in this question set and do not predict examination results, licensure or professional competence.",
    includedAccess: "Included practice", sepsisQuizCardTitle: "Sepsis Quiz", sepsisQuizCardBody: "A separate 10-question knowledge check covering early recognition, qSOFA, Time Zero, and escalation.", openSepsisQuiz: "Open Sepsis Quiz", sepsisQuizQuestions: "10 questions · score out of 10",
    referencesEyebrow: "Reference library", referencesTitle: "All sources, in one place", referencesBody: "Browse the source library used to develop scenarios and original practice questions.", referenceSearchLabel: "Search references", referenceSearchPlaceholder: "Search by title or publisher", referenceFilterAll: "All", referenceFilterScenarios: "Scenarios", referenceFilterQuestions: "Question bank", referenceFilterSepsis: "Sepsis Quiz", usedInScenarios: "Scenarios", usedInQuestions: "Question bank", usedInSepsisQuiz: "Sepsis Quiz", sourceUnavailable: "No public source link", noReferencesFound: "No references match this search.", sourceRecords: "source records",
    ...
    boundaries: "Where the platform fits", boundariesBody: "Supports personal practice, facilitated debrief and structured discussion alongside supervision, local policy and formal education.", accountModel: "Public beta access",
    accountModelBody: accountsAvailable ? "An account is optional. Guests keep progress in this browser; signed-in learners can privately sync eligible completed learning progress across devices. The AI assistant remains temporarily unavailable." : "The beta works without an account. Learning progress remains on the visitor's device; account sync and the AI assistant are temporarily unavailable.",
  ...
  ar: {
    ...
    menu: "فتح قائمة التنقل", closeMenu: "إغلاق قائمة التنقل", openTools: "فتح الأدوات", closeTools: "إغلاق الأدوات", language: "اللغة", learner: accountsAvailable ? "حساب التعلم" : "تجربة الضيف", localProfile: "ملف التعلم", localProfileShort: "هذا الجهاز", guestBetaEyebrow: "نسخة تجريبية عامة", guestBetaTitle: "لا تحتاج إلى حساب", guestBetaBody: accountsAvailable ? "استخدم الموقع كضيف، أو سجّل الدخول لمزامنة تقدم التعلم المكتمل بصورة خاصة بين أجهزتك. يبقى المساعد الذكي غير متاح مؤقتاً." : "يبقى تقدمك داخل هذا المتصفح. مزامنة الحساب والمساعد الذكي غير متاحين مؤقتاً إلى أن تكتمل ضوابطهما التشغيلية.", sendFeedback: "أرسل ملاحظتك",
    privacyStrip: accountsAvailable ? "نسخة تجريبية عامة · تسجيل الدخول اختياري. يبقى تقدم الضيف على هذا الجهاز، ويمكن للمسجل مزامنة التقدم المكتمل بصورة خاصة. الذكاء الاصطناعي متوقف مؤقتاً. لا تدخل بيانات مرضى." : "نسخة تجريبية عامة · للضيف فقط. يبقى التقدم على هذا الجهاز، وتسجيل الحسابات والذكاء الاصطناعي متوقفان مؤقتاً. تعليم أولي فقط—لا تدخل بيانات مرضى ولا تستخدمه لرعاية مريض.", simulation: "محاكاة", eyebrow: "تدرّب على الحكم السريري", homeDecisionTitle: "هل تلاحظ علامة الخطر؟", homeDecisionBody: "اقرأ المؤشرات، رتّب أولوياتك، ثم اختر الاستجابة التمريضية الأنسب.", practiceCase: "حالة تدريبية", decisionPath: "مسار الحالة", decision: "القرار", startFirstDecision: "ابدأ القرار الأول", quickLinks: "روابط سريعة", homeQuestionsHint: "تدرّب باختبارات قصيرة ومتنوعة", homeLearningHint: "تابع تقدمك وحدد نقاط التحسين",
    assistantComingSoonTitle: "المساعد الذكي", assistantComingSoonLabel: "قريبًا", assistantComingSoonBody: "اسأل عن موضوع تمريضي عام. سيبحث المساعد عن مصادر حديثة ويشرح الإجابة بالعربية والإنجليزية، مع روابط تقدر ترجع لها.",
    ...
    bilingualBody: "بدّل التجربة كاملة من دون أن تفقد موضعك.", privateDemo: accountsAvailable ? "ضيف أو حساب" : "تجربة الضيف", privateDemoBody: accountsAvailable ? "جرّب كل السيناريوهات وبنوك الأسئلة بلا حساب، أو سجّل الدخول لمزامنة التقدم المكتمل بين أجهزتك." : "جرّب كل السيناريوهات وبنوك الأسئلة بلا حساب. يبقى التقدم داخل هذا المتصفح فقط.",
    ...
    libraryEyebrow: "مكتبة السيناريوهات", libraryTitle: "اختر موقفاً سريرياً للتدرب", libraryBody: "تتبع كل حالة رحلة متغيرة. أكمل القرارات، ثم راجع المنطق وراء كل استجابة.",
    ...
    learningEyebrow: "ملف تعلمك", learningTitle: "شاهد نمط تدريبك", learningBody: "تلخص لوحة التعلم السيناريوهات المكتملة وتوضح المجالات التي قد تستفيد من مزيد من التدريب.",
    ...
    localStorageBody: accountsAvailable ? "يحتفظ هذا المتصفح باللغة وجلسة الأسئلة المؤقتة النشطة على هذا الجهاز. عند تسجيل الدخول يمكن مزامنة سجلات السيناريوهات ومجموعات الأسئلة المكتملة المؤهلة مع حساب تعلم خاص؛ ويبقى تقدم الضيف على الجهاز ما لم تختر دمجه صراحة. لا تدخل بيانات مرضى أو ملاحظات سريرية حرة، وسجّل الخروج وامسح بيانات الموقع بعد استخدام جهاز مشترك." : "يحتفظ هذا المتصفح على هذا الجهاز باللغة ومعرّفات التعلم المكتمل وجلسة الأسئلة المؤقتة النشطة. لا تتم مزامنة أي شيء مع حساب في النسخة العامة التجريبية. لا تدخل بيانات مرضى أو نصاً سريرياً حراً، وامسح بيانات الموقع بعد استخدام جهاز مشترك.",
    clearHistory: "مسح سجل التعلم", clearHistoryConfirm: "هل تريد مسح جميع محاولات التعلم المكتملة من هذا الجهاز؟ ستبقى اللغة المختارة وجلسات التدريب النشطة محفوظة.", historyCleared: "تم مسح سجل التعلم مع الاحتفاظ باللغة المختارة وجلسات التدريب النشطة.", historyClearPending: "يجري مسح سجل التعلم. جلسة التدريب النشطة محفوظة؛ حاول إنهاءها مجدداً بعد لحظات.",
    questionBankEyebrow: "تدريب على اختبارات ترخيص التمريض", questionBankTitle: "تدريب محوسب مع شرح واضح للقرار", questionBankBody: "اختر مساراً دراسياً تمريضياً، ثم استخدم التدريب الموجّه أو كوّن مجموعة ثابتة بنفسك. يظهر شرح ثنائي اللغة بعد تثبيت الإجابة.",
    ...
    quizProgress: "تقدم الأسئلة", quizScore: "درجة التدريب", correctAnswers: "الإجابات الصحيحة", quizDebriefEyebrow: "تحليل بنك الأسئلة", quizDebriefTitle: "راجع ما فهمته وما يحتاج إلى عودة", quizIncomplete: "أكمل مجموعة الأسئلة الحالية لعرض التحليل.", localExamProgress: "سجل تدريب الأسئلة", localExamProgressBody: "راجع مجموعات الأسئلة المكتملة وعدد الإجابات والأداء في كل مجال تعلّم.", setsCompleted: "المجموعات المكتملة", questionsAnswered: "الأسئلة المجاب عنها", categoryInsights: "مراجعة مجالات التعلم", morePractice: "دقة أقل في هذه العينة", developingKnowledge: "أداء متباين في هذه العينة", strongKnowledge: "دقة أعلى في هذه العينة", noExamAttempts: "ستظهر أول مجموعة أسئلة مكتملة هنا.",
    originalPracticeNotice: "تدريب مستقل ومؤلف أصلاً فقط. لا يستخدم أسئلة اختبار متذكَّرة أو سرية أو رسمية، ولا تتنبأ الدرجات بنتيجة اختبار أو ترخيص أو كفاءة.", allDifficultyLevels: "كل المستويات", questionsAvailable: "سؤالاً متاحاً", practiceSet: "مجموعة تدريب", newVariation: "تنويع جديد", viewQuestionBank: "فتح بنك الأسئلة", uniqueItems: "أسئلة فريدة", completedSetsEvidence: "مجموعات مكتملة", earlyIndicator: "مؤشر تعلم مبكر", categoryEvidenceBody: "لا يظهر مؤشر لمجال التعلم إلا بعد ثلاثة أسئلة فريدة على الأقل عبر مجموعتين مكتملتين.", contextVariant: "سياق التدريب", contextDetails: "تفاصيل السياق", contextVariantNote: "يغير السياق طريقة العرض فقط؛ وتبقى القرائن السريرية والدرجة وأفضل استجابة متسقة.", guidedScenarioTitle: "ترتيب موجّه للسيناريوهات", guidedScenarioBody: "تعيد القرارات المكتملة ترتيب المكتبة لتظهر مجالات التعلم ذات الدقة الأقل أو اختيارات السلامة قرب الأعلى بصورة أكثر تكراراً، مع إبقاء السيناريوهات الجديدة ضمن التناوب.", recommendedNext: "مقترح تالٍ", recommendationExplore: "توسيع خط الأساس", recommendationEvidence: "جمع أدلة إضافية", recommendationDevelopment: "التدرب على الأداء المتفاوت", recommendationReview: "مراجعة مجال منخفض الدقة", recommendationSafety: "مراجعة اختيار متعلق بالسلامة", adaptiveLearningTitle: "التدريب التالي المقترح", adaptiveLearningBody: "تستخدم الاقتراحات النشاط المكتمل على هذا الجهاز لتركيز جلسة التدريب التالية.", openRecommendedScenario: "فتح السيناريو المقترح", startGuidedQuestions: "بدء أسئلة موجّهة", examNonAffiliation: "فرضيات تمريضية مورد تعليمي مطوّر بصورة مستقلة. لا يصدر عن أي جهة تنظيمية تمريضية أو مالك اختبار أو مزود تقديم اختبارات، ولا ترعاه أو تؤيده أو تعتمده أو تديره أيٌّ منها. ولا يتضمن أسئلة اختبار متذكَّرة أو سرية. تصف درجات التدريب أداء المتعلم في هذه المجموعة فقط، ولا تتنبأ بنتيجة اختبار أو بالحصول على ترخيص أو بالكفاءة المهنية.",
    includedAccess: "تدريب متاح", sepsisQuizCardTitle: "اختبار Sepsis", sepsisQuizCardBody: "اختبار مستقل من 10 أسئلة عن الاكتشاف المبكر وqSOFA وTime Zero والتصعيد.", openSepsisQuiz: "فتح اختبار Sepsis", sepsisQuizQuestions: "10 أسئلة · النتيجة من 10",
    referencesEyebrow: "مكتبة المراجع", referencesTitle: "كل المراجع في مكان واحد", referencesBody: "تصفح مكتبة المصادر المستخدمة في إعداد السيناريوهات والأسئلة التدريبية الأصلية.", referenceSearchLabel: "البحث في المراجع", referenceSearchPlaceholder: "ابحث بالعنوان أو الجهة", referenceFilterAll: "الكل", referenceFilterScenarios: "السيناريوهات", referenceFilterQuestions: "بنك الأسئلة", referenceFilterSepsis: "اختبار Sepsis", usedInScenarios: "السيناريوهات", usedInQuestions: "بنك الأسئلة", usedInSepsisQuiz: "اختبار Sepsis", sourceUnavailable: "لا يوجد رابط عام للمصدر", noReferencesFound: "لا توجد مراجع مطابقة لهذا البحث.", sourceRecords: "مرجعاً",
    ...
    boundaries: "مكان المنصة", boundariesBody: "تدعم الممارسة الشخصية والنقاش الميسّر والمراجعة المنظمة إلى جانب الإشراف والسياسات المحلية والتعليم الرسمي.", accountModel: "دخول النسخة العامة التجريبية",
    accountModelBody: accountsAvailable ? "الحساب اختياري. يبقى تقدم الضيف في هذا المتصفح، ويمكن للمسجل مزامنة تقدم التعلم المكتمل المؤهل بصورة خاصة بين أجهزته. يبقى المساعد الذكي غير متاح مؤقتاً." : "تعمل النسخة التجريبية بلا حساب. يبقى تقدم التعلم على جهاز الزائر، بينما مزامنة الحساب والمساعد الذكي غير متاحين مؤقتاً.",
    ...
    footerLine: "تعلّم. استدل. اعتنِ.", privacy: "الخصوصية", terms: "الشروط", contact: "التواصل والسلامة", copyright: "فرضيات تمريضية. نسخة عامة تجريبية.",
  },
};
...
const POLICY_PAGES = {
  en: {
    privacy: {
      ...
      lead: accountsAvailable ? "This working notice describes the public testing beta with optional learning accounts. Guests can continue without signing in; signed-in learners may privately sync eligible completed learning progress. The AI assistant remains disabled while its safeguards are completed." : "This working notice describes the current guest-only public testing beta. Account registration, cloud sync and the AI assistant are disabled while qualified Saudi legal and privacy review continues.",
      effective: "Updated 16 September 2026 · public testing notice under legal review",
      ...
      sections: [
        ...
        { title: "Information used", body: accountsAvailable ? "The browser stores the selected language, scenario/question/option identifiers, completed attempts and the active timed session on this device. If you choose an account, the authentication service uses the email address and identity shared by Google, and the site may sync your preferred language, learning-record identifiers, selected option identifiers, content versions and completion timestamps. Scores and learning insights are recalculated in the app; the active timed session is not synced. Google does not give the site your Google password. The medication-math page can accept numeric values for arithmetic; those entries remain only in page memory until reset or navigation and are not saved to learning history. The beta has no free-text assistant or payment collection. Do not enter patient data, clinical notes, payment data, licence numbers or employer information." : "The browser stores the selected language, scenario/question/option identifiers, completed attempts and the active timed session on this device. The medication-math page can accept a calculation mode, exact order unit, weight when required, ordered amount or rate, medicine amount, and product or prepared-solution volume for arithmetic; its practice mode uses fixed fictional values. These numeric calculator entries remain only in page memory until reset or navigation and are not saved to learning history. Do not enter names, record numbers or clinical notes. The current beta has no account registration, cloud sync, free-text assistant or payment collection. Scores and learning-domain signals are recalculated from the current authored content. Do not enter patient data, payment data, licence numbers or employer information." },
        { title: "Device and account storage", body: accountsAvailable ? "Browser storage is not encrypted and can be visible to anyone using the same browser profile. Guest progress stays on the device unless you explicitly choose to merge it after sign-in. Signed-in account and eligible completed learning records are stored by Supabase for private cross-device sync and are restricted to the signed-in user. Use the learning dashboard to clear completed history, sign out on shared devices, and clear browser site data when finished." : "Browser storage is not encrypted and can be visible to anyone using the same browser profile. Use the learning dashboard to clear completed history, and clear browser site data after using a shared device. The current guest flow creates no authentication token or synced learning row." },
        { title: "Usage indicators without direct identifiers", body: "To understand this public beta, the site sends a limited event when a page opens or a practice set starts or finishes. It records the content area, language and a score total in daily aggregate counters; it does not send selected answers, calculator values, free text, names, IP addresses, browser identifiers or patient information to the project's analytics database. A device-only tab marker helps count approximate visit sessions. Do Not Track and Global Privacy Control signals disable these events. Counts are approximate, not a count of distinct people; small groups may still allow inference from context. The owner dashboard withholds score averages until at least five completions." },
        { title: "Feature availability", body: accountsAvailable ? (releaseFeatures.emailPasswordAuth ? "Learning accounts and cloud sync are optional. Google or email/password may be used for authentication. The AI learning assistant remains unavailable, and the site does not accept free-text clinical questions or file uploads." : "Learning accounts and cloud sync are optional. Google is used for authentication. The AI learning assistant remains unavailable, and the site does not accept free-text clinical questions or file uploads.") : "Account registration, cloud learning sync and the AI learning assistant are not available in this public testing beta. The site does not accept free-text clinical questions or upload files." },
        ...
        { title: "Purpose and retention", body: accountsAvailable ? "Device data supports the requested educational experience and remains until you clear learning history or browser site data. Account identity and synced learning records remain until you clear the account's learning history or request account deletion. Anonymous daily usage counters help improve the beta and are deleted after 90 days; they are stored by OpenAI Sites/Cloudflare and may be processed outside Saudi Arabia. Account authentication and learning sync are provided through Supabase and may be processed outside Saudi Arabia. Support emails are processed through Gmail while the final retention schedule remains under review." : "Device data supports the requested educational experience and remains until you clear learning history or browser site data. Anonymous daily usage counters help improve the beta and are deleted after 90 days; they are stored by OpenAI Sites/Cloudflare and may be processed outside Saudi Arabia. Only the site owner can view their aggregate dashboard. OpenAI Sites hosting logs follow the provider's settings. Support emails are processed through Gmail to respond to reports while a final retention schedule remains under review." },
        { title: "Your controls and requests", body: accountsAvailable ? "The learning dashboard lets you clear completed learning history; browser controls can remove local site data. Signed-in users can export their current learning data and sign out at any time. To request account deletion or raise a privacy concern, email Scenario.hq90@gmail.com without patient or workplace-confidential information. The project aims to acknowledge privacy requests within five business days; applicable statutory deadlines govern the final response." : "The learning dashboard lets you clear completed learning history; browser controls can remove all site data. To raise a privacy concern, email Scenario.hq90@gmail.com without patient or workplace-confidential information. The project aims to acknowledge privacy requests within five business days; applicable statutory deadlines govern the final response." },
      ],
    },
    terms: {
      ...
      effective: "Effective 9 September 2026 · draft pending legal review",
      ...
      sections: [
        { title: "Educational scope", body: "The website offers fictional scenarios and independently authored practice questions for self-study and facilitated discussion. The medication calculators perform arithmetic from user-entered values for either a single dose or an IV infusion and may display mL/dose or mL/hr. Named options provide context or populate stated label concentrations only. The calculators do not choose a dose, infer a missing unit, validate an order or preparation, program an infusion pump, assess tubing or device effects, or establish safe administration. The separate practice mode grades arithmetic against fixed fictional problems. The website does not provide medical advice, professional supervision, a credential, continuing-education credit, licensure eligibility, a competency decision or a prediction of any examination result." },
        ...
        { title: accountsAvailable ? "Optional learning account" : "Guest-only release", body: accountsAvailable ? "You may use the beta as a guest or sign in to sync eligible completed learning records. Guest progress is merged only when you explicitly select that option. The AI assistant remains temporarily unavailable. Do not enter patient or workplace-confidential information anywhere on the site." : "Account registration, cloud sync and the AI assistant are temporarily unavailable. Learning progress remains in this browser. Do not enter patient or workplace-confidential information anywhere on the site." },
        ...
        { title: "Availability", body: accountsAvailable ? "Learning accounts are optional and free during the beta. The beta does not offer subscriptions, checkout or payment collection. Draft content may be corrected, withdrawn or unavailable without notice." : "The beta does not offer accounts, subscriptions, checkout or payment collection. Draft content may be corrected, withdrawn or unavailable without notice." },
        { title: "Audience and jurisdiction", body: "This public testing beta is intended for adult nursing learners and educators, including users in Saudi Arabia. Governing-law, dispute and liability clauses remain open for qualified legal review before a wider or paid release." },
      ],
    },
    contact: {
  ...
  ar: {
    privacy: {
      ...
      lead: accountsAvailable ? "يصف إشعار العمل هذا النسخة العامة التجريبية مع حسابات تعلم اختيارية. يستطيع الضيف المتابعة بلا تسجيل، ويمكن للمسجل مزامنة تقدم التعلم المكتمل المؤهل بصورة خاصة. يبقى المساعد الذكي معطلاً إلى أن تكتمل ضوابطه." : "يصف إشعار العمل هذا النسخة العامة التجريبية الحالية للضيف فقط. تسجيل الحسابات والمزامنة السحابية والمساعد الذكي معطلة أثناء استمرار المراجعة القانونية ومراجعة الخصوصية السعودية المؤهلة.",
      effective: "حُدّث في 16 سبتمبر 2026 · إشعار تجربة عامة قيد المراجعة القانونية",
      ...
      sections: [
        ...
        { title: "المعلومات المستخدمة", body: accountsAvailable ? "يحفظ المتصفح على هذا الجهاز اللغة ومعرّفات السيناريوهات والأسئلة والخيارات والمحاولات المكتملة والجلسة المؤقتة النشطة. إذا اخترت الحساب تستخدم خدمة المصادقة البريد والهوية اللذين يشاركهما Google، وقد يزامن الموقع لغتك المفضلة ومعرّفات سجلات التعلم ومعرّفات الخيارات المختارة وإصدارات المحتوى وأوقات الإكمال. يعيد التطبيق حساب الدرجات ومؤشرات التعلم، ولا يزامن جلسة الأسئلة المؤقتة النشطة. لا يحصل الموقع على كلمة مرور Google. يمكن للحاسبة استقبال قيم رقمية لإجراء العملية الحسابية؛ وتبقى داخل ذاكرة الصفحة حتى المسح أو الانتقال ولا تُحفظ في سجل التعلم. لا يوجد مساعد نص حر أو تحصيل مدفوعات في النسخة الحالية. لا تدخل بيانات مرضى أو ملاحظات سريرية أو بيانات دفع أو رقم ترخيص أو جهة عمل." : "يحفظ المتصفح على هذا الجهاز اللغة ومعرّفات السيناريوهات والأسئلة والخيارات والمحاولات المكتملة والجلسة المؤقتة النشطة. يمكن لصفحة الحساب الدوائي استقبال نوع العملية ووحدة الأمر الدقيقة والوزن عند الحاجة والكمية أو المعدل المطلوب وكمية الدواء وحجم المنتج أو المحلول المحضّر لإجراء العملية الحسابية، بينما يستخدم وضع التدريب قيماً خيالية ثابتة. تبقى مدخلات الحاسبة الرقمية في ذاكرة الصفحة حتى المسح أو الانتقال ولا تُحفظ في سجل التعلم. لا تدخل أسماء أو أرقام ملفات أو ملاحظات سريرية. لا تتضمن النسخة الحالية تسجيل حساب أو مزامنة سحابية أو مساعداً للنص الحر أو تحصيل مدفوعات. لا تدخل بيانات مرضى أو بيانات دفع أو رقم ترخيص أو جهة عمل." },
        { title: "تخزين الجهاز والحساب", body: accountsAvailable ? "تخزين المتصفح غير مشفر وقد يراه من يستخدم ملف المتصفح نفسه. يبقى تقدم الضيف على الجهاز ما لم تختر دمجه صراحة بعد الدخول. تحفظ Supabase هوية الحساب وسجلات التعلم المكتملة المؤهلة للمزامنة الخاصة بين الأجهزة، وتُقيد للمستخدم المسجل نفسه. استخدم لوحة التعلم لمسح السجل، وسجّل الخروج وامسح بيانات الموقع بعد استخدام جهاز مشترك." : "تخزين المتصفح غير مشفر وقد يراه من يستخدم ملف المتصفح نفسه. استخدم لوحة التعلم لمسح السجل المكتمل، وامسح بيانات الموقع بعد استخدام جهاز مشترك. لا ينشئ مسار الضيف الحالي رمز مصادقة أو صف تعلم متزامناً." },
        { title: "مؤشرات استخدام بلا معرّفات مباشرة", body: "لفهم تجربة النسخة العامة، يرسل الموقع حدثاً محدوداً عند فتح صفحة أو بدء تدريب أو إكماله. تُحفظ منطقة المحتوى واللغة ومجموع النتيجة في عدادات يومية مجمّعة؛ ولا تُرسل الإجابات المختارة أو قيم الحاسبة أو النصوص الحرة أو الأسماء أو عناوين IP أو معرّفات المتصفح أو بيانات المرضى إلى قاعدة إحصاءات المشروع. تساعد علامة داخل تبويب الجهاز فقط على تقدير جلسات الزيارة. تتوقف هذه الأحداث عند تفعيل إشارات Do Not Track أو Global Privacy Control. الأعداد تقريبية وليست عدداً لأشخاص فريدين؛ وقد تتيح المجموعات الصغيرة استنتاجات من السياق. تحجب لوحة المالك متوسط النتائج إلى أن تتوفر خمس إكمالات على الأقل." },
        { title: "توفر الميزات", body: accountsAvailable ? (releaseFeatures.emailPasswordAuth ? "حساب التعلم والمزامنة السحابية اختياريان. يمكن استخدام Google أو البريد وكلمة المرور للمصادقة. يبقى مساعد التعلم بالذكاء الاصطناعي غير متاح، ولا يستقبل الموقع أسئلة سريرية بنص حر أو ملفات مرفوعة." : "حساب التعلم والمزامنة السحابية اختياريان. يستخدم Google للمصادقة. يبقى مساعد التعلم بالذكاء الاصطناعي غير متاح، ولا يستقبل الموقع أسئلة سريرية بنص حر أو ملفات مرفوعة.") : "لا يتوفر تسجيل الحسابات أو مزامنة التعلم السحابية أو مساعد التعلم بالذكاء الاصطناعي في النسخة العامة التجريبية. ولا يستقبل الموقع أسئلة سريرية بنص حر أو ملفات مرفوعة." },
        ...
        { title: "الغرض والاحتفاظ", body: accountsAvailable ? "تدعم بيانات الجهاز التجربة التعليمية المطلوبة وتبقى حتى تمسح سجل التعلم أو بيانات الموقع من المتصفح. تبقى هوية الحساب وسجلات التعلم المتزامنة حتى تمسح سجل الحساب أو تطلب حذف الحساب. تساعد عدادات الاستخدام اليومية المجهولة على تحسين النسخة التجريبية وتُحذف بعد ٩٠ يوماً، وقد تعالج OpenAI Sites/Cloudflare وSupabase البيانات خارج السعودية. تُعالج رسائل الدعم عبر Gmail بينما يبقى جدول الاحتفاظ النهائي قيد المراجعة." : "تدعم بيانات الجهاز التجربة التعليمية المطلوبة وتبقى حتى تمسح سجل التعلم أو بيانات الموقع من المتصفح. تساعد عدادات الاستخدام اليومية المجهولة على تحسين النسخة التجريبية، وتُحذف بعد ٩٠ يوماً؛ وتحفظها استضافة OpenAI Sites/Cloudflare وقد تُعالج خارج السعودية. لا يطلع على لوحتها المجمّعة إلا مالك الموقع. تتبع سجلات الاستضافة إعدادات المزود. وتُعالج رسائل الدعم عبر Gmail للرد على البلاغ بينما يبقى جدول الاحتفاظ النهائي قيد المراجعة." },
        { title: "ضوابطك وطلباتك", body: accountsAvailable ? "تتيح لوحة التعلم مسح سجل التعلم المكتمل، وتستطيع ضوابط المتصفح إزالة بيانات الجهاز. يستطيع المستخدم المسجل تصدير بيانات تعلمه الحالية وتسجيل الخروج في أي وقت. لطلب حذف الحساب أو رفع ملاحظة خصوصية راسل Scenario.hq90@gmail.com من دون بيانات مريض أو معلومات عمل سرية. يستهدف المشروع تأكيد الاستلام خلال خمسة أيام عمل، مع بقاء المدد النظامية المطبقة حاكمة للاستجابة النهائية." : "تتيح لوحة التعلم مسح سجل التعلم المكتمل، وتستطيع ضوابط المتصفح إزالة جميع بيانات الموقع. لرفع ملاحظة خصوصية راسل Scenario.hq90@gmail.com من دون بيانات مريض أو معلومات عمل سرية. يستهدف المشروع تأكيد استلام طلب الخصوصية خلال خمسة أيام عمل، مع بقاء المدد النظامية المطبقة حاكمة للاستجابة النهائية." },
      ],
    },
    terms: {
      ...
      effective: "يسري من 9 سبتمبر 2026 · مسودة بانتظار المراجعة القانونية",
      ...
      sections: [
        { title: "النطاق التعليمي", body: "يقدم الموقع سيناريوهات خيالية وأسئلة تدريبية مؤلفة بصورة مستقلة للتعلم الذاتي والنقاش الميسّر. تنفذ حاسبات الدواء عملية حسابية من القيم التي يدخلها المستخدم لجرعة مفردة أو تسريب وريدي، وقد تعرض mL/dose أو mL/hr. وتقدم الخيارات المسماة سياقاً أو تعبئ تركيزات الملصق المعلنة فقط. ولا تختار الحاسبات جرعة، ولا تستنتج وحدة مفقودة، ولا تعتمد أمراً أو تحضيراً، ولا تبرمج مضخة تسريب، ولا تقيّم أثر الأنابيب أو الجهاز، ولا تثبت سلامة الإعطاء. ويقارن وضع التدريب المنفصل العمليات الحسابية بمسائل خيالية ثابتة. ولا يقدم الموقع نصيحة طبية أو إشرافاً مهنياً أو اعتماداً أو ساعات تعليم مستمر أو أهلية ترخيص أو قرار كفاءة أو توقعاً لنتيجة أي اختبار." },
        ...
        { title: accountsAvailable ? "حساب تعلم اختياري" : "إصدار للضيف فقط", body: accountsAvailable ? "يمكنك استخدام النسخة كضيف أو تسجيل الدخول لمزامنة سجلات التعلم المكتملة المؤهلة. لا يُدمج تقدم الضيف إلا إذا اخترت ذلك صراحة. يبقى المساعد الذكي غير متاح مؤقتاً. لا تدخل معلومات مريض أو بيانات عمل سرية في أي موضع من الموقع." : "تسجيل الحساب والمزامنة السحابية والمساعد الذكي غير متاحة مؤقتاً. يبقى تقدم التعلم داخل هذا المتصفح. لا تدخل معلومات مريض أو بيانات عمل سرية في أي موضع من الموقع." },
        ...
        { title: "التوفر", body: accountsAvailable ? "حسابات التعلم اختيارية ومجانية خلال النسخة التجريبية. لا تقدم النسخة اشتراكات أو شراء أو تحصيل مدفوعات. وقد يُصحح المحتوى الأولي أو يُسحب أو يتعذر دون إشعار." : "لا تقدم النسخة التجريبية حسابات أو اشتراكات أو شراء أو تحصيل مدفوعات. وقد يُصحح المحتوى الأولي أو يُسحب أو يتعذر دون إشعار." },
        { title: "الجمهور والنطاق النظامي", body: "النسخة العامة التجريبية مخصصة للمتعلمين والمعلّمين البالغين في التمريض، ومنهم مستخدمون في السعودية. ما تزال بنود النظام الحاكم والنزاعات والمسؤولية مفتوحة لمراجعة قانونية مؤهلة قبل إصدار أوسع أو مدفوع." },
      ],
    },
    contact: {
...
const NAV_ITEMS = [["home", "home", House], ["scenarios", "simulations", Exam], ["questions", "questionBank", ClipboardText], ["sepsis-quiz", "sepsisQuiz", FirstAidKit], ["dose-practice", "dosePractice", Calculator], ["learning", "learning", ChartLineUp], ["resources", "resources", Books], ["about", "about", Info]];
const DESKTOP_NAV_ITEMS = NAV_ITEMS.filter(([path]) => ["scenarios", "questions", "sepsis-quiz", "learning"].includes(path));
const PRIMARY_NAV_ITEMS = NAV_ITEMS.filter(([path]) => ["scenarios", "questions", "learning"].includes(path));
const TOOL_NAV_ITEMS = NAV_ITEMS.filter(([path]) => ["dose-practice", "resources", "about"].includes(path));
...
function activeRoutePath(route) {
...
function routeTitleKey(route) {
  ...
  if (route.page === "dose-practice") return "dosePractice";
  return route.page;
}
...
function Header({ lang, setLanguage, route, menuOpen, setMenuOpen, t }) {
  ...
  const active = activeRoutePath(route);
  const dockActive = active === "home" ? "scenarios" : active === "sepsis-quiz" ? "questions" : active;
  useEffect(() => {
    ...
    <header className="site-header site-header-v2">
      <div className="header-inner">
        <AppLink to="home" className="brand" onNavigate={() => setMenuOpen(false)} aria-label={PRODUCT_NAME[lang]}>
          ...
          <span className="brand-copy"><span className="brand-name">{PRODUCT_NAME[lang]}</span><span className="brand-tagline">{t("footerLine")}</span></span>
        </AppLink>
        <nav className="desktop-nav" aria-label={lang === "ar" ? "التنقل الرئيسي" : "Primary navigation"}>
          {DESKTOP_NAV_ITEMS.map(([path, label]) => <AppLink key={path} to={path} className={active === path ? "active" : ""} aria-current={active === path ? "page" : undefined}>{t(label)}</AppLink>)}
        </nav>
        <div className="header-actions">
          ...
          <button ref={menuButtonRef} type="button" className="tools-button" onClick={() => setMenuOpen((open) => !open)} aria-expanded={menuOpen} aria-controls="tools-navigation" aria-label={menuOpen ? t("closeTools") : t("openTools")}><Toolbox size={20} weight="duotone" aria-hidden="true" /><span>{t("tools")}</span></button>
        </div>
      </div>
      <nav id="tools-navigation" className={`tools-navigation ${menuOpen ? "open" : ""}`} aria-label={t("tools")}>
        {TOOL_NAV_ITEMS.map(([path, label, Icon]) => <AppLink key={path} to={path} onNavigate={() => setMenuOpen(false)} className={active === path ? "active" : ""} aria-current={active === path ? "page" : undefined}><Icon size={21} aria-hidden="true" /><span>{t(label)}</span></AppLink>)}
      </nav>
    </header>
    <nav className="mobile-dock" aria-label={lang === "ar" ? "التنقل الرئيسي للهاتف" : "Mobile primary navigation"}>
      {PRIMARY_NAV_ITEMS.map(([path, label, Icon]) => <AppLink key={path} to={path} className={dockActive === path ? "active" : ""} aria-current={dockActive === path ? "page" : undefined}><Icon size={22} weight={dockActive === path ? "fill" : "duotone"} aria-hidden="true" /><span>{t(label)}</span></AppLink>)}
    </nav>
  </>;
}
...
function ScenarioExperience({ scenario, lang, t, session, setSession, onComplete }) {
  ...
  return <div className="scenario-shell"><PatientRail scenario={scenario} step={step} lang={lang} t={t} /><section className="decision-stage">
    ...
    <div className="decision-question"><div className="question-heading"><span className="question-icon"><Target size={28} weight="duotone" /></span><div><p className="eyebrow">{t("clinicalMoment")}</p><h1 ref={questionRef} tabIndex="-1">{localize(step.question, lang)}</h1><p className="translated-question" lang={alternateLang} dir={alternateLang === "ar" ? "rtl" : "ltr"}>{localize(step.question, alternateLang)}</p></div></div><p className="question-instruction">{t("questionIntro")}</p>
      ...
      {confirmed && selectedChoice ? <div className={`decision-feedback ${selectedChoice.classification}`} aria-live="polite">{(() => { const Icon = classificationIcon(selectedChoice.classification); return <Icon size={28} weight="fill" aria-hidden="true" />; })()}<div><p className="feedback-label">{t("feedback")} · {t(selectedChoice.classification)}</p><h2>{localize(selectedChoice.feedback, lang)}</h2><p>{localize(selectedChoice.rationale, lang)}</p><small><LockKey size={14} />{t("selectionLocked")}</small></div></div> : null}
...
const allQuestionReferences = [
  ...new Map(
    [...examReferences, ...references, ...SEPSIS_QUIZ_REFERENCES].map((reference) => [reference.id, reference]),
  ).values(),
];
const scenarioReferenceIds = new Set(scenarios.flatMap((scenario) => [
  ...(scenario.referenceIds ?? []),
  ...scenario.steps.flatMap((step) => step.referenceIds ?? []),
  ...(scenario.hypotheses ?? []).flatMap((hypothesis) => hypothesis.referenceIds ?? []),
]));
const questionReferenceIds = new Set(questionBank.flatMap((question) => question.referenceIds ?? []));
const sepsisReferenceIds = new Set(SEPSIS_QUIZ_REFERENCES.map((reference) => reference.id));
...
function QuestionBankPage({ lang, t, examProfile, onComplete, storageKey, historyClearPending }) {
  ...
  if (!quiz && !result) {
    return <div className="page-container question-bank-page"><SectionIntro eyebrow={t("questionBankEyebrow")} title={t("questionBankTitle")} body={t("questionBankBody")} />
      <section className="sepsis-quiz-entry" aria-labelledby="sepsis-quiz-entry-title">
        <span className="sepsis-quiz-entry__icon" aria-hidden="true"><FirstAidKit size={32} weight="duotone" /></span>
        <div><p className="eyebrow">SEPSIS</p><h2 id="sepsis-quiz-entry-title">{t("sepsisQuizCardTitle")}</h2><p>{t("sepsisQuizCardBody")}</p><small>{t("sepsisQuizQuestions")}</small></div>
        <AppLink to="sepsis-quiz" className="button button-primary">{t("openSepsisQuiz")}{lang === "ar" ? <ArrowLeft size={17} /> : <ArrowRight size={17} />}</AppLink>
      </section>
  ...
  if (result) {
    ...
    return <div className="page-container quiz-result-page"><SectionIntro eyebrow={t("quizDebriefEyebrow")} title={t("quizDebriefTitle")} body={t("localExamProgressBody")} />{notice ? <p className="form-notice result-notice" role="status"><Clock size={18} weight="fill" />{notice}</p> : null}<section className="result-summary"><ScoreRing score={result.score} lang={lang} /><div className="result-primary"><span>{t("quizScore")}</span><h2>{formatNumber(Math.round(result.score), lang)}%</h2></div><dl className="result-metrics"><div><dt>{t("correctAnswers")}</dt><dd>{formatNumber(result.correctCount, lang)} / {formatNumber(result.totalQuestionCount, lang)}</dd></div><div><dt>{t("selectTrack")}</dt><dd>{localize(examTracks.find((track) => track.id === result.examId)?.shortLabel, lang)}</dd></div></dl></section>
      ...
      <div className="debrief-actions"><button type="button" className="button button-secondary" onClick={resetQuiz}><Exam size={18} weight="duotone" />{t("restartQuiz")}</button><AppLink to="learning" className="button button-secondary">{t("continueLearning")}</AppLink></div>
    </div>;
  }
  ...
  return <div className="page-container active-quiz-page"><div className="active-quiz-top"><div><span className="simulation-badge"><ClipboardText size={18} weight="bold" />{localize(examTracks.find((track) => track.id === activeExamId)?.shortLabel, lang)}</span><AccessBadge t={t} /></div><div className={`exam-timer ${remainingSeconds <= 60 ? "urgent" : ""}`} aria-live="off"><span><Clock size={19} weight="duotone" />{t("timeRemaining")}</span><strong>{formatCountdown(remainingSeconds, lang)}</strong><small>{t("sessionSaved")}</small></div><div className="quiz-progress-copy"><strong>{t("question")} {formatNumber(quiz.index + 1, lang)} {t("of")} {formatNumber(quiz.questions.length, lang)}</strong><span>{formatNumber(percentage, lang)}%</span></div><div className="progress-meter" role="progressbar" aria-label={t("quizProgress")} aria-valuemin="0" aria-valuemax="100" aria-valuenow={percentage}><span style={{ width: `${percentage}%` }} /></div></div>
    <ExamQuestion question={currentQuestion} questionNumber={quiz.index + 1} language={lang} translate={t} selectedOptionId={selectedAnswerId} locked={isCurrentAnswerLocked} notice={notice} isLastQuestion={quiz.index === quiz.questions.length - 1} onSelect={(optionId) => { selectAnswer(optionId); setNotice(""); }} onPrimaryAction={isCurrentAnswerLocked ? advanceQuestion : lockAnswer} />
  </div>;
}

function GuestBetaPanel({ t }) {
  return <section className="account-panel account-panel-muted"><LockKey size={34} weight="duotone" aria-hidden="true" /><div><p className="eyebrow">{t("guestBetaEyebrow")}</p><h2>{t("guestBetaTitle")}</h2><p>{t("guestBetaBody")}</p></div></section>;
}

function LearningPage({ lang, t, profile, examProfile, onClearHistory, onStart, auth, syncStatus, onSignOut, onExportLearningData, accountsEnabled }) {
  ...
  return <div className="page-container learning-page"><SectionIntro eyebrow={t("learningEyebrow")} title={t("learningTitle")} body={t("learningBody")} />
    {accountsEnabled ? <Suspense fallback={null}><AccountPanel auth={auth} lang={lang} syncStatus={syncStatus} onSignOut={onSignOut} onExportLearningData={onExportLearningData} emailPasswordEnabled={releaseFeatures.emailPasswordAuth} /></Suspense> : <GuestBetaPanel t={t} />}
    ...
    <section className="local-profile-note"><Database size={31} weight="duotone" /><div><h2>{t("localStorageTitle")}</h2><p>{t("localStorageBody")}</p>{historyCleared ? <p className="clear-history-status" role="status">{t("historyCleared")}</p> : null}</div><button type="button" className="button clear-history-button" onClick={clearHistory} disabled={historyClearing || (!profile.attempts.length && !examProfile.attempts.length)}><Trash size={17} aria-hidden="true" />{t("clearHistory")}</button></section>
  </div>;
}
...
function ReferencesPage({ lang, t }) {
  ...
  const filteredReferences = useMemo(() => {
    ...
    return allQuestionReferences.filter((reference) => {
      const isScenarioSource = scenarioReferenceIds.has(reference.id);
      const isQuestionSource = questionReferenceIds.has(reference.id);
      const isSepsisSource = sepsisReferenceIds.has(reference.id);
      if (filter === "scenarios" && !isScenarioSource) return false;
      if (filter === "questions" && !isQuestionSource) return false;
      if (filter === "sepsis" && !isSepsisSource) return false;
      if (!normalizedQuery) return true;
      const searchable = [reference.title?.en, reference.title?.ar, reference.organization?.en, reference.organization?.ar, reference.year]
        .filter(Boolean)
        .join(" ")
  ...
  const filters = [
    ["all", "referenceFilterAll"],
    ["scenarios", "referenceFilterScenarios"],
    ["questions", "referenceFilterQuestions"],
    ["sepsis", "referenceFilterSepsis"],
  ];
  return <div className="page-container references-page"><SectionIntro eyebrow={t("referencesEyebrow")} title={t("referencesTitle")} body={t("referencesBody")} />
    <div className="reference-toolbar"><label className="reference-search"><span>{t("referenceSearchLabel")}</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("referenceSearchPlaceholder")} /></label><div className="reference-filters" role="group" aria-label={t("referenceSearchLabel")}>{filters.map(([value, label]) => <button key={value} type="button" className={filter === value ? "active" : ""} aria-pressed={filter === value} onClick={() => setFilter(value)}>{t(label)}</button>)}</div></div>
    <p className="reference-summary"><strong>{formatNumber(filteredReferences.length, lang)}</strong> {t("sourceRecords")}</p>
    {filteredReferences.length ? <div className="source-rows">{filteredReferences.map((reference) => { const isScenarioSource = scenarioReferenceIds.has(reference.id); const isQuestionSource = questionReferenceIds.has(reference.id); const isSepsisSource = sepsisReferenceIds.has(reference.id); return <article className="full-source-row" key={reference.id}><div className="source-year">{reference.year}</div><div className="source-details"><span>{localize(reference.organization, lang)}</span><h2>{localize(reference.title, lang)}</h2><div className="reference-usage">{isScenarioSource ? <span>{t("usedInScenarios")}</span> : null}{isQuestionSource ? <span>{t("usedInQuestions")}</span> : null}{isSepsisSource ? <span>{t("usedInSepsisQuiz")}</span> : null}</div>{reference.accessNote || reference.licensingNote ? <details><summary>{t("accessNote")}</summary>{reference.accessNote ? <p>{localize(reference.accessNote, lang)}</p> : null}{reference.licensingNote ? <p>{localize(reference.licensingNote, lang)}</p> : null}</details> : null}</div>{reference.url ? <a href={reference.url} target="_blank" rel="noopener noreferrer nofollow" className="button button-secondary source-link"><span>{t("source")}</span><ArrowSquareOut size={18} /><span className="sr-only">{t("opensNewTab")}</span></a> : <span className="reference-source-unavailable">{t("sourceUnavailable")}</span>}</article>; })}</div> : <p className="reference-empty" role="status">{t("noReferencesFound")}</p>}
  </div>;
}
...
function AboutPage({ t }) {
  return <div className="page-container about-page"><SectionIntro eyebrow={t("aboutEyebrow")} title={t("aboutTitle")} body={t("aboutLead")} /><div className="about-grid"><section><Target size={29} weight="duotone" /><div><h2>{t("purpose")}</h2><p>{t("purposeBody")}</p></div></section><section><Brain size={29} weight="duotone" /><div><h2>{t("method")}</h2><p>{t("methodBody")}</p></div></section><section><Info size={29} weight="duotone" /><div><h2>{t("boundaries")}</h2><p>{t("boundariesBody")}</p></div></section><section><LockKey size={29} weight="duotone" /><div><h2>{t("accountModel")}</h2><p>{t("accountModelBody")}</p></div></section></div><section className="editorial-band"><div><p className="eyebrow">{t("resources")}</p><h2>{t("editorial")}</h2></div><p>{t("editorialBody")}</p></section></div>;
}
...
export function App() {
  ...
  const auth = useAuthSession({ enabled: releaseFeatures.learningAccounts });
  ...
  useEffect(() => {
    ...
    if (!["confirmed", "recovery", "google"].includes(authMarker)) return;
  ...
  else if (route.page === "sepsis-quiz") {
    page = <Suspense fallback={<div className="page-container">{lang === "ar" ? "جارٍ تحميل اختبار Sepsis…" : "Loading the Sepsis Quiz…"}</div>}><SepsisQuiz lang={lang} /></Suspense>;
  }
  ...
  return <div className="app-shell"><Header lang={lang} setLanguage={setLanguage} route={route} menuOpen={menuOpen} setMenuOpen={setMenuOpen} t={t} auth={auth} syncStatus={syncStatus} accountsEnabled={releaseFeatures.learningAccounts} /><main id="main-content" ref={mainRef} tabIndex="-1">{page}</main>{releaseFeatures.nursingAssistant ? <Suspense fallback={null}><NursingAssistant lang={lang} /></Suspense> : null}<Footer lang={lang} t={t} /></div>;
}
