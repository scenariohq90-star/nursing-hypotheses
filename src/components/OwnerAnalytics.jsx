...
function summarize(rows, scores) {
  ...
  for (const row of scores) {
    if (Number(row.count) < 5) continue;
    const key = `${row.event}:${row.dimension}`;
    const current = byEvent.get(key);
    if (current) current.scoreSum = Number(row.scoreSum) || 0;
  }
  ...
  const examRows = ["saudi-nursing", "international-rn", "computerized-practice"].map((dimension) => ({ dimension, started: get("question_start", dimension).count, completed: get("question_complete", dimension).count, unavailable: get("question_unavailable", dimension).count, scoreSum: get("question_complete", dimension).scoreSum })).filter((item) => item.started || item.completed || item.unavailable);
  const sepsis = { dimension: "sepsis", started: get("sepsis_start", "sepsis").count, completed: get("sepsis_complete", "sepsis").count, unavailable: 0, scoreSum: get("sepsis_complete", "sepsis").scoreSum };
...
export function OwnerAnalytics({ lang }) {
  ...
  const [state, setState] = useState({ status: "loading", rows: [], scores: [], through: "" });
  useEffect(() => {
    ...
    fetch(`/api/owner-analytics?days=${days}`, { signal: controller.signal, credentials: "same-origin", cache: "no-store" })
      .then(async (response) => {
        ...
        return { status: "ready", rows: Array.isArray(payload.rows) ? payload.rows : [], scores: Array.isArray(payload.scores) ? payload.scores : [], through: payload.through ?? "" };
      })
      .then((next) => { if (!controller.signal.aborted) setState(next); })
      .catch(() => { if (!controller.signal.aborted) setState({ status: "error", rows: [], scores: [], through: "" }); });
    return () => controller.abort();
  }, [days, revision]);
  const summary = useMemo(() => summarize(state.rows ?? [], state.scores ?? []), [state.rows, state.scores]);
  const chartDays = useMemo(() => {
    const end = Date.parse(`${state.through}T00:00:00Z`);
    if (!Number.isFinite(end)) return summary.visitsByDay;
    const visitMap = new Map(summary.visitsByDay);
    return Array.from({ length: days }, (_, index) => {
      const day = new Date(end - (days - index - 1) * 86_400_000).toISOString().slice(0, 10);
      return [day, visitMap.get(day) ?? 0];
    });
  }, [days, state.through, summary.visitsByDay]);
  const maxDaily = Math.max(1, ...chartDays.map(([, count]) => count));
  ...
  return <div className="page-container owner-analytics" dir={lang === "ar" ? "rtl" : "ltr"}>
    ...
    {state.status === "ready" ? <>
      ...
      {summary.views === 0 && summary.starts === 0 ? <p className="oa-state">{copy.empty}</p> : <>
        <div className="oa-grid"><section className="oa-panel oa-trend"><div className="oa-panel-heading"><ChartLineUp size={22} /><h2>{copy.active}</h2></div><div className="oa-bars" role="img" aria-label={copy.trendCaption}>{chartDays.map(([day, count]) => <div key={day} className="oa-bar-item" title={`${day}: ${count}`}><span style={{ height: count === 0 ? "0" : `${Math.max(8, count / maxDaily * 100)}%`, visibility: count === 0 ? "hidden" : "visible" }} /><small>{day.slice(5)}</small></div>)}</div><p>{copy.trendCaption}</p></section><section className="oa-panel"><h2>{copy.languages}</h2><div className="oa-simple-rows">{summary.visitsByLanguage.map(([language, count]) => <div key={language}><span>{language === "ar" ? "العربية" : "English"}</span><strong>{format(count, lang)}</strong></div>)}</div></section></div>
        <div className="oa-grid"><section className="oa-panel"><h2>{copy.scenarios}</h2><div className="oa-list">{summary.scenarioRows.length ? summary.scenarioRows.map((item) => <div key={item.dimension}><strong>{scenarioLabel(item.dimension)}</strong><span>{copy.started}: {format(item.started, lang)} · {copy.completed}: {format(item.completed, lang)}</span><small>{copy.average}: {displayScore(item, lang, copy)}</small></div>) : <p>—</p>}</div></section><section className="oa-panel"><h2>{copy.exams}</h2><div className="oa-list">{summary.examRows.length ? summary.examRows.map((item) => <div key={item.dimension}><strong>{examLabel(item.dimension)}</strong><span>{copy.started}: {format(item.started, lang)} · {copy.completed}: {format(item.completed, lang)}{item.unavailable ? ` · ${copy.unavailableStarts}: ${format(item.unavailable, lang)}` : ""}</span><small>{copy.average}: {displayScore(item, lang, copy)}</small></div>) : <p>—</p>}</div></section></div>
